...
def train(args):
    ...
    checkpoints_dir = os.path.join(workspace, 'checkpoints', pretrained_ckpt.split("/")[-1],filename, 
    ...
    statistics_path = os.path.join(workspace, 'statistics', pretrained_ckpt.split("/")[-1],filename, 
        model_type, dataset_choice, 'loss_type={}'.format(loss_type), 
        'augmentation={}'.format(augmentation), 
        'max_note_shift={}'.format(max_note_shift), 
    ...
    logs_dir = os.path.join(workspace, 'logs', pretrained_ckpt.split("/")[-1], filename, 
        model_type, dataset_choice, 'loss_type={}'.format(loss_type), 
        'augmentation={}'.format(augmentation), 
        'max_note_shift={}'.format(max_note_shift), 
        'batch_size={}'.format(batch_size))
    ...
    logging.info('Loading checkpoint {}'.format(pretrained_ckpt))
    checkpoint = torch.load(pretrained_ckpt)
    iteration = 0
    
    if 'model' in checkpoint and 'note_model' in checkpoint['model']:
