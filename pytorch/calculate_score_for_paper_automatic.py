...
class ScoreCalculator(object):
    ...
    def calculate_score_per_song(self, args):
        ...
        if self.evaluate_frame:
            ...
            # print("huh?", metrics)
            tmp = sklearn.metrics.precision_recall_fscore_support(y_true.flatten(), y_pred.flatten())
...
def calculate_metrics(args, thresholds=None):
    ...
    out_dict = {}
    out_dict['ckpt_path'] = args.checkpoint_path
    for key in stats_dict.keys():
        print('{}: {:.4f}'.format(key, np.mean(stats_dict[key])))
        out_dict[key] = np.mean(stats_dict[key])
    #save out_dict to a file
...
if __name__ == '__main__':
    parser_metrics = argparse.ArgumentParser(description='')
    ...
    parser_metrics.add_argument('--checkpoint_folder', type=str, required=True)
    parser_metrics.add_argument('--checkpoint_names', type=str, required=True)
    ...
    parser_metrics.add_argument('--cuda', action='store_true', default=False)
